      ...
      <#if Adapter.hasAddingCommentRight()>
        <div class="actions">
          <#if Adapter.hasWriteRightOnComment(comment)>
            <a href="#"
               onclick="$.get('@comment/${comment.id}/@delete', function(data) {displayNotification('${Context.getMessage('label.activity.comment.deleted')}');});">${Context.getMessage('label.action.Delete')}</a>
            |
          </#if>
